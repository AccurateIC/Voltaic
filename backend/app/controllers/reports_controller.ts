...
import { getPdfPropertyBetweenValidator } from "../validators/archive.js";
...
export default class ReportsController {
  ...
  async generateDummy({ request, response }: HttpContext) {
    ...
    try {
      ...
      console.log("propertyNames from request:", propertyNames);

      // console.log("Fetched propertyData length:", propertyData.length);
  ...
  `;
      ...
      // function generateTypstBarChart(xs: string[], ys: number[], label: string) {
      ...
      //     #lq.diagram(
      //       xaxis: (
      //         ticks: xs.map(rotate.with(-45deg, reflow: true)).map(align.with(right)).enumerate()),
      //       yaxis: (
      //         range: (${yMin}, ${yMax})
      //       ),
      //       lq.bar(range(ys.len()), ys, label: ["${label}"])
      //     )
      //   `;
      // }
