...
import { PieChart, Pie, Cell, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Label } from "recharts";
        ...
        <div className="bg-gray-800 p-8 rounded-lg shadow-lg flex justify-center">
          <div>
            ...
            <LineChart width={300} height={250} data={lineData}>
              ...
              <XAxis dataKey="hour">
                <Label value="Time (in hours)" position="bottom" offset={0} className="text-white" />
              </XAxis>
              <YAxis dataKey="count">
                <Label value="Anomaly Count" angle={-90} position="left middle" offset={-15} className="text-white" />
              </YAxis>
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="count" label="Anamoly in a Day" stroke="#42A5F5" />
            </LineChart>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-8 rounded-lg shadow-lg flex justify-center">
          <div>
            ...
            <LineChart width={300} height={250} data={weekData}>
              <CartesianGrid stroke="#ccc" />
              <XAxis dataKey="type">
                <Label value="Type of Anamoly" position="bottom" offset={0} className="text-white" />
              </XAxis>
              <YAxis dataKey="count">
                <Label value="Anomaly Count" angle={-90} position="left middle" offset={-15} className="text-white" />
              </YAxis>
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="count" label="Anamoly in a Week" stroke="#FFFF66" />
            </LineChart>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between mt-6 gap-4">
        <div className="relative w-full md:w-auto">
          <button
            ...
            className="bg-white px-6 py-2 text-sm 2xl:text-xl text-black font-bold rounded-md w-full md:w-auto shadow-[inset_4px_4px_10px_0px_#00000040]">
            ADD FILTER ▼
          </button>
          {showFilter && (
