...
const Anomalies = () => {
  ...
  const fetchNotifications = async () => {
    ...
    if (!selectedEntry || !selectedEntry.startedAt || !selectedEntry.finishedAt || !selectedEntry.archive?.gensetProperty?.propertyName) {
...
useEffect(() => {
  const fetchPropertyData = async () => {
    ...
    try {
      setIsLoading(true);
      const url = `${import.meta.env.VITE_ADONIS_BACKEND}/archive/getPropertyDataBetween?from=${from}&to=${to}&propertyName=${propertyName}`;
  ...
  labelFormatter={(label) =>
    `Time: ${DateTime.fromMillis(label).toFormat("HH:mm:ss")}`
  }
/>
              <Legend verticalAlign="top" height={36} />
              <Line
