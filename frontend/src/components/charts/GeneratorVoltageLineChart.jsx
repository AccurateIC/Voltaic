...
export const GeneratorVoltageLineChart = ({
  ...
  useEffect(() => {
    ...
    // Show notification once when anomaly is detected for the first time and remove if anomaly state changes to false
    if (l1IsAnomaly && !l1NotificationRef.current) {
    ...
    // Remove notification if anomaly becomes false
    if (!l1IsAnomaly && l1NotificationRef.current) {
      setNotifications((prevNotifications) =>
        prevNotifications.filter((notification) => notification.id !== "L1")
      );
      l1NotificationRef.current = false; // Reset flag
    }
    ...
    if (!l2IsAnomaly && l2NotificationRef.current) {
      ...
      l2NotificationRef.current = false; // Reset flag
    }
    ...
    if (!l3IsAnomaly && l3NotificationRef.current) {
      ...
      l3NotificationRef.current = false; // Reset flag
    }
