...
export const GeneratorCurrentLineChart = ({
  ...
  const dispatch = useDispatch();
  const currentData = useSelector(selectCurrentData); // Access current data from Redux
  ...
  useEffect(() => {
    // Dispatch current data to Redux
    if (l1Current !== 0 && l2Current !== 0 && l3Current !== 0) {
      dispatch(
        addCurrentData({
          time: new Date(timeStamp).toLocaleTimeString(),
    ...
    // Handle notifications for current anomalies (L1, L2, L3)
    if (l1CIsAnomaly && !l1NotificationRef.current) {
      dispatch(
        addNotification({
          id: "L1",
