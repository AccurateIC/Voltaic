/**
 * Anomalies UI (route `anomalies`). Uses notification/archive-specific
 * endpoints.
 */
...
export const Anomalies = () => {
  ...
  // keep the state, but also sync to ref whenever it changes
 ...
 const fetchNotifications = async (page = 1, showLoading = true) => {
  ...
  try {
    ...
    // ignore old responses — only use the latest one
  ...
  const handleResolveAll = async () => {
    ...
    try {
      ...
      const resolvedCount = notificationIds.length;

      // instantly update the navbar badge (React Query cache)
      queryClient.setQueryData(["notifications-count"], (old) => {
        if (!old) return old;
        return {
          ...old,
          anomaly: Math.max(0, (old.anomaly || 0) - resolvedCount),
        };
      });
...
useMessageBus(TransmitChannels.NOTIFICATION, () => {
  ...
  // if 5 seconds passed since last update → fire immediately
  ...
  const remainingTime = 5000 - timeSinceLastFire;
