...
export const generateBaseColor = () => {
  ...
  return {
    h,
    s: generateRandomNumber(0.3, 0.45), // Slightly reduced saturation range
    l: generateRandomNumber(0.5, 0.65), // Adjusted lightness for better visibility
  };
};
...
const adjustColorForDarkTheme = (color, role) => {
  ...
  switch (role) {
    case "base":
      newColor = { h: color.h, s: Math.min(color.s + 0.1, 0.55), l: 0.45 };
      break;
    case "light":
      newColor = {
        h: (color.h + 5) % 360, // Slight hue shift
        s: Math.max(color.s - 0.05, 0.3),
        l: 0.6,
      };
      break;
    case "dark":
...
export const generateColorPalette = (color) => {
  ...
  return {
    ...
    light: { h: base.h, s: Math.max(base.s - 0.15, 0.25), l: Math.min(base.l + 0.2, 0.8) },
    // Darker variant with slightly increased saturation
    dark: { h: base.h, s: Math.min(base.s + 0.1, 0.55), l: Math.max(base.l - 0.2, 0.35) },
    // Complement using golden angle for more interesting combinations
    complement: { h: (base.h + goldenAngle) % 360, s: Math.min(base.s + 0.05, 0.5), l: Math.min(base.l + 0.05, 0.7) },
    // Accent using another golden angle shift
    accent: { h: (base.h + goldenAngle * 2) % 360, s: Math.min(base.s + 0.1, 0.5), l: Math.max(base.l - 0.05, 0.45) },
  };
};
