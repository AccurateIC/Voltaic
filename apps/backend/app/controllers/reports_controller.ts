  ...
  async generateDummy({ request, response }: HttpContext) {
    ...
    try {
      ...
      const generateTypstBarChart = (
        ...
        // Fix for single value: add dummy tick/bar
        const isSingle = ys.length === 1;
        const xsFixed = isSingle ? [...xs, ""] : xs;
        const ysFixed = isSingle ? [...ys, 0] : ys;
        const barWidth = isSingle ? 0.2 : 0.7;
...
#align(center)[
  #box(width: 100%, height: 300pt)[
    #lq.diagram(
      ...
      ylabel: "${title}",
      xaxis: (
      ticks: (${xsFixed.map((val) => `"${val}"`).join(",")},)
          .map(rotate.with(-45deg, reflow: true))
          .map(align.with(right))
          .enumerate(),
      ),
      lq.bar(
        range(${ysFixed.length}),
        ( ${ysFixed.join(", ")}, ),
        label: ["${title}"],
        width: ${barWidth},
        fill: rgb("#2563eb")
